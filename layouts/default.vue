<template>
  <div id="my-scrollbar">
    <Nuxt />
    <div class="contacts">
      <a
        :href="whatsApp"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat on WhatsApp"
      >
        <svg
          height="512"
          viewBox="0 0 176 176"
          width="512"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
        >
          <linearGradient
            id="linear-gradient"
            gradientUnits="userSpaceOnUse"
            x1="88"
            x2="88"
            y1=".72"
            y2="167.48"
          >
            <stop offset="0" stop-color="#35db23" />
            <stop offset="1" stop-color="#27a11a" />
          </linearGradient>
          <g id="Layer_2" data-name="Layer 2">
            <g id="_08.whatsapp" data-name="08.whatsapp">
              <rect
                id="background"
                fill="url(#linear-gradient)"
                height="176"
                rx="16.33"
                width="176"
              />
              <g id="icon" fill="#fff">
                <path
                  d="m126.8 49.2a54.57 54.57 0 0 0 -87.42 63.13l-5.79 28.11a2.08 2.08 0 0 0 .33 1.63 2.11 2.11 0 0 0 2.24.87l27.55-6.53a54.56 54.56 0 0 0 63.09-87.21zm-8.59 68.56a42.74 42.74 0 0 1 -49.22 8l-3.84-1.9-16.89 4 .05-.21 3.5-17-1.88-3.71a42.72 42.72 0 0 1 7.86-49.59 42.73 42.73 0 0 1 60.42 0 2.28 2.28 0 0 0 .22.22 42.72 42.72 0 0 1 -.22 60.19z"
                />
                <path
                  d="m116.71 105.29c-2.07 3.26-5.34 7.25-9.45 8.24-7.2 1.74-18.25.06-32-12.76l-.17-.15c-12.09-11.21-15.23-20.54-14.47-27.94.42-4.2 3.92-8 6.87-10.48a3.93 3.93 0 0 1 6.15 1.41l4.45 10a3.91 3.91 0 0 1 -.49 4l-2.25 2.92a3.87 3.87 0 0 0 -.35 4.32c1.26 2.21 4.28 5.46 7.63 8.47 3.76 3.4 7.93 6.51 10.57 7.57a3.82 3.82 0 0 0 4.19-.88l2.61-2.63a4 4 0 0 1 3.9-1l10.57 3a4 4 0 0 1 2.24 5.91z"
                />
              </g>
            </g>
          </g>
        </svg>
      </a>
      <a :href="mail">
        <svg
          version="1.1"
          id="Capa_1"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
          x="0px"
          y="0px"
          viewBox="0 0 512 512"
          style="enable-background: new 0 0 512 512"
          xml:space="preserve"
        >
          <circle style="fill: #5f98d1" cx="256" cy="256" r="256" />
          <path
            style="fill: #3a6da1"
            d="M415.813,147.466H95.558l116.019,120.806l33.48,33.9c-18.216-4.164-19.343-6.759-27.415-13.349
	c-4.234-3.457-12.414-12.852-24.838-25.54C154.023,223.682,88.217,156.484,88.217,156.484v198.822l8.265,8.265l-0.925,0.963
	L242.68,511.657c4.412,0.226,8.852,0.343,13.32,0.343c141.385,0,256-114.615,256-256c0-4.246-0.11-8.466-0.313-12.661
	L415.813,147.466z"
          />
          <g>
            <polygon
              style="fill: #ffffff"
              points="88.217,156.484 88.217,355.306 185.175,255.493 	"
            />
            <polygon
              style="fill: #ffffff"
              points="423.783,156.484 423.783,355.306 327.307,255.895 	"
            />
            <path
              style="fill: #ffffff"
              d="M95.558,147.466h320.255L289.948,278.524c-8.958,9.327-21.331,14.599-34.262,14.599
		s-25.304-5.272-34.262-14.599L95.558,147.466z"
            />
            <path
              style="fill: #ffffff"
              d="M297.209,285.496c-10.799,11.244-25.933,17.694-41.523,17.694c-15.589,0-30.724-6.448-41.522-17.693
		l-21.349-22.23L95.558,364.534h320.255l-97.256-101.267L297.209,285.496z"
            />
          </g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
          <g></g>
        </svg>
      </a>
      <div
        class="contacts__wraper"
        v-for="(con, index) of contacts"
        :key="index"
        @click="btnAtionInfo(con.content)"
      >
        <a :href="con.href" class="contacts-a">
          <div class="contacts-a__action">
            <div class="name">
              {{ con.name }}
            </div>
            <div class="hoverText">
              {{ con.hoverText }}
            </div>
          </div>
        </a>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      contacts: [
        {
          name: 'Imprint',
          href: '#imprint',
          hoverText: 'Imprint',
          content: 'imprint',
        },
        {
          name: 'Privacy',
          href: '#privacy',
          hoverText: 'Privacy',
          content: 'privacy',
        },
      ],
      whatsApp: 'https://wa.me/491701790979',
      mail: 'mailto:info@lcc24h.com',
    }
  },
  computed: {
    infoModal() {
      return this.$store.state.activeInfo
    },
    showinfo() {
      return this.$store.state.showinfo
    },
  },
  methods: {
    closeInfo() {
      this.$store.commit('set_showinfo', false)
    },
    async btnAtionInfo(content) {
      const getData = await this.$content(content).fetch()
      this.$store.commit('set_activeInfo', getData)
      this.$store.commit('set_showinfo', true)
    },
  },
}
</script>

<style>
</style>